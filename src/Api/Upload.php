...
class Upload extends File
{
	 ...
	 * @param string|int $remotePath 远程文件路径或远程目录ID(int是ID，string路径)
	 * @return array
	 */
    function upload(string $filePath,$remotePath=0): array {
        //预上传
	    $remoteDirId=is_int($remotePath)?$remotePath:0;
	    $remoteDirName=is_string($remotePath)?$remotePath:null;
        $preview=$this->create($remoteDirName?:basename($filePath), md5_file($filePath), filesize($filePath), $remoteDirId, (bool)$remoteDirName);
     ...
     * @return array|mixed
     */
    function create(string $filename, string $md5, int $size, int $parentFileID=0,bool $containDir=false)
    {
