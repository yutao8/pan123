...
//2.创建文件夹(如果不存在)
$file_list = $sdk->file->list_v2(0, 0, 10, $dirName, 1);
$dir=current(array_filter($file_list['fileList'],fn($v)=>$v['filename']===$dirName && $v['type']===1));
if (empty($dir['fileId'])) {
	//创建目录
...
//8.获取下载链接
$res = $sdk->file->download($fileID);
var_dump("获取下载链接成功，链接：" . $res['downloadUrl']);
